...
//	tryRotate = tryHarder = false;
    ...
    void createMaps(cv::Mat& mapXY, int outputSize, bool horizontal, bool inverse) {
        ...
        if(offsetMap.cols != outputSize) {
            ...
            constexpr const int vec_size = cv::float32::nlanes;
            constexpr size_t simd_alignment = cv::float32::nlanes * sizeof(float);
            ...
            cv::float32 indexAdd = cv::v_load_aligned(tmp2);
            float indexMul = 2.0 / float(outputSize - 1);

            for (; i <= outputSize - vec_size; i += vec_size) {

                cv::float32 v_val = (cv::v_setall_f32(static_cast<float>(i)) + indexAdd) * cv::v_setall_f32(indexMul);
